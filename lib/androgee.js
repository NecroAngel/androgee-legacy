...
class Androgee {
  constructor () {
    ...
    this.sub.subscribe('MinecraftUsers')
          ...
          if (res.body.length > reply.length) {
            ...
            try {
              if (playerListDif.length >= 4) {
                ...
                console.log(msg)
                // self.discordConnection.client.channels.find('id', process.env.MODDED_MINECRAFT_CHANNEL).sendMessage(msg)
              }
            } catch (err) {
              console.log('The diff function failed: ' + err)
            }
          }
          self.redis.publish('MinecraftUsers', res.body)
        }).connect()
