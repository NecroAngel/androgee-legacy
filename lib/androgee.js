...
class Androgee {
  ...
  initRedis () {
    ...
    this.redis.set('terrariaPlayers', 'There are 0/20 players online:', Redis.print)
    this.redis.set('gmodPlayers', 'There are 0/20 players online:', Redis.print)
  }
          ...
          if (res.body.length > reply.length) {
            ...
            try {
              if (playerListDif.length >= 4) {
                ...
                console.log(msg)
              // self.discordConnection.client.channels.find('id', process.env.MINECRAFT_CHANNEL).setTopic(msg) // This may be useful in the future
