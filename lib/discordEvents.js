        ...
        case message.content.includes('~minecraft'): {
          if (message.channel.id === process.env.MINECRAFT_CHANNEL || message.channel.id === process.env.DEBUG_CHANNEL) {
            ...
            switch (true) {
              case message.content.includes('time'):
                if (message.member.roles.find('id', process.env.HEROES_ROLE)) {
                  const timeNormalized = message.content.slice(16).toLowerCase()
                  const commandFactory = new CommandFactory()
                  if (timeNormalized === 'day' || timeNormalized === 'night') {
                    commandFactory.setMinecraftTime(timeNormalized)
                    message.reply('New time has been set')
                  } else {
                    message.reply('You have to enter the command like this: ' + '``~minecraft time day``')
                  }
                } else {
                  message.reply('You must have the **Heroes** role to perform this action.')
                }
                break
              case message.content.includes('toggledownfall'): {
                if (message.member.roles.find('id', process.env.HEROES_ROLE)) {
                  const commandFactory = new CommandFactory()
                  commandFactory.setMinecraftWeather()
                  message.reply('Weather has been toggled')
                } else {
                  message.reply('You must have the **Heroes** role to perform this action.')
                }
                break
