  ...
  constructor (rustConnection, androgee) {
    ...
    rustConnection.on('message', function (msg) {
      try {
        ...
        } else if (msg.message.includes('[EAC]' || msg.message.includes('ProjectileHack'))) {
          const rustChannel = androgee.discordConnection.client.channels.find('id', process.env.DEBUG_CHANNEL)
          rustChannel.sendMessage(msg.message)
        }
      } catch (err) {
